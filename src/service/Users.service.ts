...
import { RegisterUser, Login, ForgotPwd } from '../type/user';
...
public async _checkIfUserExists(user: RegisterUser): Promise<any> {
    ...
    try {
      ...
      if (emailCheck) {
        ...
        returnValue = {
          status: true,
          reason: 'User not exists in the Database table.'
        };
      }
      return returnValue;
    } catch (err) {
...
/**
   * @param  {string} email user's email
   * @param  {string} password password required to login account.
   * @returns Promise<User>
   */
  public async login(user: Login): Promise<any> {
    ...
    try {
      ...
      if (activeUser === undefined) {
        response = 'Please register yourself first';
      } else {
  ...
  public async forgotPwd(user: ForgotPwd): Promise<any> {
    const userRepository = getManager().getCustomRepository(UsersRepo);
    let response;
    try {
      const activeUser = await userRepository.findUserByEmailId(
