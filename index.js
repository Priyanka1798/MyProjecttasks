...
// Display booking homepage(1. /homepage api)(create reminder for booking an appointment in salon....for people.)
app.get('/', function(req, res) {
    ...
    res.render('home', {
        date : defaultDT.format('YY-MM-DD'),
        time : defaultDT.format('HH:mm')
    });
});

// Process an incoming booking(2. create reminder for booking purpose -  /book api)
app.post('/book', function(req, res) {
    ...
    messagebird.lookup.read(req.body.number, process.env.COUNTRY_CODE, function (err, response) {
        ...
        if (response.type != "mobile") {
            ...
            // Schedule reminder 3 hours prior to the treatment(update reminder)
            var reminderDT = appointmentDT.clone().subtract({hours: 3});

            // Send scheduled message with API
            messagebird.messages.create({
                originator : "BeautyHub Salon",
